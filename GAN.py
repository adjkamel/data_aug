...
class Model_GAN(object):
    ...
    def save(self, num):
        ...
        with open("models_CGAN/gen.json", "w+") as json_file:
            json_file.write(gen_json)

        with open("models_CGAN/dis.json", "w+") as json_file:
            json_file.write(dis_json)

        self.GAN.G.save_weights("models_CGAN/gen"+str(num)+".h5")
        self.GAN.D.save_weights("models_CGAN/dis"+str(num)+".h5")
    ...
    def load(self, num):
        ...
        # Generator
        gen_file = open("models_CGAN/gen.json", 'r')
        gen_json = gen_file.read()
        gen_file.close()

        self.GAN.G = model_from_json(gen_json)
        self.GAN.G.load_weights("models_CGAN/gen"+str(num)+".h5")

        # Discriminator
        dis_file = open("models_CGAN/dis.json", 'r')
        dis_json = dis_file.read()
        dis_file.close()

        self.GAN.D = model_from_json(dis_json)
        self.GAN.D.load_weights("models_CGAN/dis"+str(num)+".h5")
